...
class content_model extends model {
    ...
    public function add_content($data,$isimport = 0) {
        ...
        if($model_tablename!='zbdp'){
            if(!empty($systeminfo['glgg'])){
                //文章股票代码表
                $this->table_name = $this->db_tablepre.$this->model[$modelid]['tablename'].'_stockcode';
                foreach(explode(",",$systeminfo['glgg']) as $v){
                    $this->insert(array("atid"=>$id,"stockcode"=>$v));
                }
            }
            
            if(!empty($systeminfo['keywords'])){
               //文章关键词表
                $this->table_name = $this->db_tablepre.$this->model[$modelid]['tablename'].'_keyword';
                foreach(explode(",",$systeminfo['keywords']) as $v){
                    $keyid=file_get_contents("http://221.232.160.243/bolanadmin/insertKey.php?keyword=".$v);
                    $this->insert(array("atid"=>$id,"keyid"=>$keyid));
                }
            }
			
			//文章便签分类表
            if(!empty($modelinfo['labels'])){
                $this->table_name = $this->db_tablepre.$this->model[$modelid]['tablename'].'_labels';
                foreach(explode(",",$modelinfo['labels']) as $v){
                    if(!empty($v)){
                        $this->insert(array("atid"=>$id,"flid"=>$v));
                    }
                }
            }
        }
        ...
        if($this->modelid==1)
        {
            $this->table_name =  $this->db_tablepre.'catandat';
            $this->insert(array("catid"=>$systeminfo["catid"],"atid"=>$id,"modid"=>$modelid));
            if($id && isset($_POST['othor_catid']) && is_array($_POST['othor_catid'])) {
                foreach ($_POST['othor_catid'] as $cid=>$_v) {
                    $this->insert(array("catid"=>$cid,"atid"=>$id,"modid"=>$modelid),false,true);
                }
            }
        }
        //如果是行业研究下的栏目数据，添加一条资讯到行业研究栏目下
        $IndustryId=include '/home/wwwroot/default/bolanadmin/getIndustryId.txt';
        if(in_array($systeminfo["catid"],$IndustryId))
        {
            $this->table_name =  $this->db_tablepre.'catandat';
            $this->insert(array("catid"=>'5491',"atid"=>$id,"modid"=>$modelid),false,true);
        }
    ...
    public function edit_content($data,$id) {
        ...
        $systeminfo['description'] = str_replace(array('\\','#',"'"),' ',$systeminfo['description']);
        $systeminfo['keywords'] = str_replace(array('/','\\','#','.',"'"),' ',$systeminfo['keywords']);
        $systeminfo['username'] = !empty($data['username']) ? $data['username'] : param::get_cookie('admin_username');
        //主表
        ...
        if($model_tablename!='zbdp'){
            ...
            //文章股票代码表
            $this->table_name = $this->db_tablepre.$model_tablename.'_stockcode';
            $this->delete(array('atid'=>$id));
            if(!empty($systeminfo['glgg'])){
                foreach(explode(",",$systeminfo['glgg']) as $v){
                    $this->insert(array("atid"=>$id,"stockcode"=>$v));
                }
             ...
             //文章关键词表
            $this->table_name = $this->db_tablepre.$model_tablename.'_keyword';
            $this->delete(array('atid'=>$id));
            if(!empty($systeminfo['keywords'])){
                foreach(explode(",",$systeminfo['keywords']) as $v){
                    $keyid=file_get_contents("http://221.232.160.243/bolanadmin/insertKey.php?keyword=".$v);
			...
			//文章便签分类表
            $this->table_name = $this->db_tablepre.$model_tablename.'_labels';
            $this->delete(array('atid'=>$id));
            if(!empty($modelinfo['labels'])){
                foreach(explode(",",$modelinfo['labels']) as $v){
                    if(!empty($v)){
    ...
    public function delete_content($id,$file,$catid = 0) {
        if($this->modelid==1)
        {
            $this->table_name = "bl_catandat";
            $this->delete(array('atid'=>$id,'catid'=>$catid));
            $c=$this->count('atid='.$id);
            if($c==0)
            {
                //删除主表数据
     ...
     * @param $id 内容id
     */
    public function review_content($id) {
        $this->table_name = $this->db_tablepre.$this->model_tablename;
        $modelinfo['ispass']=1;
        $this->update($modelinfo,array('id'=>$id));
    }
